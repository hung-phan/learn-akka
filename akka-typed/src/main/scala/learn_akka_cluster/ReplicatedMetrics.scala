...
object ReplicatedMetrics {
  ...
  private case class InternalSubscribeResponse(
    rsp: SubscribeResponse[LWWMap[String, Long]]
  ) extends InternalCommand
  private case class InternalClusterMemberUp(msg: ClusterEvent.MemberUp)
      extends InternalCommand
  private case class InternalClusterMemberRemoved(
    msg: ClusterEvent.MemberRemoved
  ) extends InternalCommand
  ...
  def apply(measureInterval: FiniteDuration,
            cleanupInterval: FiniteDuration): Behavior[Command] =
    Behaviors.setup { context =>
      DistributedData
        .withReplicatorMessageAdapter[Command, LWWMap[String, Long]] {
          replicator =>
            Behaviors.withTimers[Command] { timers =>
              ...
              val memberUpRef =
                context.messageAdapter(InternalClusterMemberUp.apply)
              val memberRemovedRef =
                context.messageAdapter(InternalClusterMemberRemoved.apply)
              ...
              Behaviors.receiveMessage {
                ...
                case InternalSubscribeResponse(c @ Changed(UsedHeapKey)) =>
                  val usedHeapPercent =
                    UsedHeap(c.get(UsedHeapKey).entries.collect {
                ...
                case InternalClusterMemberRemoved(
                    ClusterEvent.MemberRemoved(m, _)
                    ) =>
                  nodesInCluster -= nodeKey(m.address)
