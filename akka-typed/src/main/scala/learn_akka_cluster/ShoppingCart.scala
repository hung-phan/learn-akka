...
object ShoppingCart {
  ...
  private case class InternalGetResponse(
    replyTo: ActorRef[Cart],
    rsp: GetResponse[LWWMap[String, LineItem]]
  ) extends InternalCommand
  private case class InternalUpdateResponse[A <: ReplicatedData](
    rsp: UpdateResponse[A]
  ) extends InternalCommand
  private case class InternalRemoveItem(
    productId: String,
    getResponse: GetResponse[LWWMap[String, LineItem]]
  ) extends InternalCommand
  ...
  def apply(userId: String): Behavior[Command] = Behaviors.setup { context =>
    DistributedData
      .withReplicatorMessageAdapter[Command, LWWMap[String, LineItem]] {
        replicator =>
          ...
          def updateCart(data: LWWMap[String, LineItem],
                         item: LineItem): LWWMap[String, LineItem] = {
