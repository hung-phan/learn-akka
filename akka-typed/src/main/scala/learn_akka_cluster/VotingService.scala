...
object VotingService {
  ...
  private case class InternalSubscribeResponse(chg: SubscribeResponse[Flag])
      extends InternalCommand
  private case class InternalUpdateResponse[A <: ReplicatedData](
    rsp: UpdateResponse[A]
  ) extends InternalCommand
  private case class InternalGetResponse(replyTo: ActorRef[Votes],
                                         rsp: GetResponse[PNCounterMap[String]])
      extends InternalCommand

  def apply(): Behavior[Command] = Behaviors.setup { context =>
    DistributedData.withReplicatorMessageAdapter[Command, Flag] {
      replicatorFlag =>
        DistributedData
          .withReplicatorMessageAdapter[Command, PNCounterMap[String]] {
            replicatorCounters =>
              ...
              replicatorFlag
                .subscribe(OpenedKey, InternalSubscribeResponse.apply)
              ...
              def start = Behaviors.receiveMessagePartial[Command] {
                ...
                case InternalSubscribeResponse(c @ Changed(OpenedKey))
                    if c.get(OpenedKey).enabled =>
                  becomeOpen()
              ...
              def becomeOpen() = {
                ...
                replicatorFlag
                  .subscribe(ClosedKey, InternalSubscribeResponse.apply)
                Behaviors.receiveMessagePartial(
                  open orElse getVotes(open = true)
                )
              }
              ...
              def open: PartialFunction[Command, Behavior[Command]] = {
                  ...
                  replicatorCounters.askUpdate(
                    askReplyTo =>
                      Update(
                        ...
                        WriteLocal,
                        askReplyTo
                      )(_.incrementBy(participant, 1)),
                    InternalUpdateResponse.apply
                  )

                  Behaviors.same

                case InternalUpdateResponse(_: UpdateSuccess[_]) =>
                  Behaviors.same
                ...
                case InternalSubscribeResponse(c @ Changed(ClosedKey))
                    if c.get(ClosedKey).enabled =>
                  Behaviors.receiveMessagePartial(getVotes(open = false))

                case InternalSubscribeResponse(Changed(OpenedKey)) =>
                  Behaviors.same
              }
              ...
              def getVotes(
                ...
                case GetVotes(replyTo) =>
                  replicatorCounters.askGet(
                    askReplyTo =>
                      Get(CountersKey, ReadAll(3.seconds), askReplyTo),
