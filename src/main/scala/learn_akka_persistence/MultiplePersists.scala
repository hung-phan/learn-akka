...
object MultiplePersists extends App {
  ...
  val system = ActorSystem("MultiplePersistsDemo", ConfigFactory.load().getConfig("learn_akka_persistence"))
  ...
  // Uncomment next line for seeding data
  ...
  accountant ! Invoice("The Supercar Company", new Date, 2000)

  // If you send some data during the period of persistence, it will stash the message until persistence is completed
}
