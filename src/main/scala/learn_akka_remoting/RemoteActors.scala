...
object RemoteActors extends App {
  val localSystem = ActorSystem(
    "LocalSystem",
    ConfigFactory.load("akka_remoting/remote_actors.conf")
  )
  val localSimpleActor =
    localSystem.actorOf(Props[SimpleActor], "localSimpleActor")
  ...
  val remoteActorSelection = localSystem.actorSelection(
  ...
  remoteActorRefFuture.onComplete {
    ...
    case Failure(ex) =>
      println(s"I failed to resolve the remove actor because: $ex")
  }

  // Method 3: actor identification via messages
  /**
    * - actor resolver will ask for an actor selection from the local actor system
    * - actor resolve will send a Identify(42) to the actor selection
    * - the remote actor will AUTOMATICALLY respond with ActorIdentity(42, actorRef)
    * - the actor resolve is free to use the actorRef
    */
  class ActorResolve extends Actor with ActorLogging {
  ...
  localSystem.actorOf(Props[ActorResolve], "localActorResolver")
}
...
object RemoteActors_Remote extends App {
  val remoteSystem = ActorSystem(
    "RemoteSystem",
    ConfigFactory
      .load("akka_remoting/remote_actors.conf")
      .getConfig("remoteSystem")
  )
  val remoteSimpleActor =
    remoteSystem.actorOf(Props[SimpleActor], "remoteSimpleActor")
