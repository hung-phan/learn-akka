...
class OysterCardValidator extends Actor with ActorLogging {
  ...
  /**
    * Store a enormous amount of data
    */
  ...
  override def receive: Receive = {
    case EntryAttempt(card @ OysterCard(id, amount), _) =>
      ...
      else sender() ! EntryRejected(s"[$id] Insufficient funds, please top up")
    case ReceiveTimeout =>
...
class TubeStation(port: Int, numberOfTurnstiles: Int) extends App {
  val config = ConfigFactory
    .parseString(s"""
       ...
       |""".stripMargin)
    .withFallback(
      ConfigFactory.load("akka_remoting/cluster_sharding_example.conf")
    )
