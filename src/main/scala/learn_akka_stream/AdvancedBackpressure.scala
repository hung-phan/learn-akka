...
object AdvancedBackpressure extends App {
  ...
  val aggregateNotificationFlow = Flow[PagerEvent]
    .conflate((event1, event2) => {
      val nInstance = event1.nInstance + event2.nInstance

      PagerEvent(s"You have $nInstance events that require your attention", new Date, nInstance)
    })
...
//    .run()
  ...
  // this will produce the element all the time and causes backpressure to kick in
