...
class AskSpec extends TestKit(ActorSystem("AskSpec")) with ImplicitSender with AnyWordSpecLike with BeforeAndAfterAll {
  ...
  def authenticatorTestSuite(props: Props) = {
    ...
    "fail to authenticate if invalid password" in {
      ...
      authManager ! Authenticate("daniel", "i_love_akka")
...
object AskSpec {
  ...
  class AuthManager extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      ...
      case Authenticate(username, password) => handleAuthenticate(username, password)
    }

    private def handleAuthenticate(username: String, password: String) = {
