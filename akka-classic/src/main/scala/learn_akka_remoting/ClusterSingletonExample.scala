...
class PaymentSystemNode(port: Int, shouldStartSingleton: Boolean = true) extends App {
  val config = ConfigFactory
    .parseString(s"""
      ...
      |""".stripMargin)
    .withFallback(
      ConfigFactory.load("akka_remoting/cluster_singleton_example.conf")
    )
...
class OnlineShopCheckout(paymentSystem: ActorRef)
    extends Actor
    with ActorLogging {
  var orderId = 0
  ...
  override def receive: Receive = {
    case Order(_, totalAmount) =>
      log.info(
        s"Receive order $orderId for amount $totalAmount, sending transaction to validate"
      )
      val newTransaction =
        Transaction(orderId, UUID.randomUUID().toString, totalAmount)
...
object OnlineShopCheckout {
  def props(paymentSystem: ActorRef) =
